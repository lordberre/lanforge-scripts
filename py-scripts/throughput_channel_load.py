...
class IPV4VariableTime(Realm):
    ...
    def start(self, print_pass=False, print_fail=False):
        ...
        if self.wait_for_ip(temp_stas,timeout_sec=60):
            print("admin-up....")
    ...
    def chn_util(self,ssh_root, ssh_passwd,channnel=0, band = None):
        # To find the channel utilization
        if band == '2.4G':
            cmd = 'iwpriv wifi0vap0 get_chutil'  # command to get channel utilization
        elif band == '5G':
            cmd = 'iwpriv wifi1vap0 get_chutil'     # command to get channel utilization
        '''elif band == '5G_High':
            cmd = 'iwpriv wifi2vap0 get_chutil'     # command to get channel utilization'''
        try:
    ...
    def re_run_traff(self, adj_trf_rate, add_sub_rate):
        ...
        if add_sub_rate == "sub":
            self.cx_profile.side_a_min_bps = int(self.cx_profile.side_a_min_bps) - int((int(self.cx_profile.side_a_min_bps) / 100) * adj_trf_rate)
            self.cx_profile.side_b_min_bps = int(self.cx_profile.side_b_min_bps) - int((int(self.cx_profile.side_b_min_bps) / 100) * adj_trf_rate)
            '''if adj_trf_rate < self.cx_profile.side_a_min_bps:
                ...
                if int(self.cx_profile.side_a_min_bps) > 10000 :
                    self.cx_profile.side_a_min_bps = int(self.cx_profile.side_a_min_bps) - 10000
                    self.cx_profile.side_b_min_bps = int(self.cx_profile.side_b_min_bps) - 10000'''
    ...
    def check_util(self,real_cli_obj = None, util_list = None, real_cli = None,
                   ssh_root = None, ssh_passwd = None,test_time = 0,up_down = [0],threshold=None,channnel=0, band = None):
        ...
        for util in util_list:  #  get throughput for every utilization values
            ...
            while util_flag:
                ...
                util_val = self.chn_util(ssh_root, ssh_passwd, channnel=channnel, band = band)  # find the channel utilization
                if (util - 3) <= util_val <= (util + 3):
                    ...
                    if util_val < (util - 3):
                        ...
                        if ((util ) - util_val) <= 4:
                            self.re_run_traff(4, "add")
                        elif ((util ) - util_val) <= 8:
                            self.re_run_traff(6, "add")
                        elif ((util ) - util_val) <= 12:
                            self.re_run_traff(8, "add")
                        elif (util ) - util_val <= 16:
                            self.re_run_traff(10, "add")
                        elif (util ) - util_val > 16:
                            self.re_run_traff(20, "add")

                    # channel utilization is less than the expected utilization value
                    elif util_val > (util + 3):
                        print("greater than {}% util...".format(util))
                        if (util_val - (util )) <= 4:
                            self.re_run_traff(4, "sub")
                        elif (util_val - (util )) <= 8:
                            self.re_run_traff(6, "sub")
                        elif (util_val - (util )) <= 12:
                            self.re_run_traff(8, "sub")
                        elif util_val - (util ) <= 16:
                            self.re_run_traff(10, "sub")
                        elif util_val - (util ) > 16:
                            self.re_run_traff(20, "sub")
...
def main():
    try:
        parser = argparse.ArgumentParser(description='''throughput.py:
    --------------------\n
    ...
    **** vap_channel, vap_radio, bands and radio should be in equal length
    **** If bands == triband, the script will automatically run all the three bands(2.4G, 5G_low, 5G_high) at the same time. 
         If bands = [2.4G,5G_low] then the test will the provided bands run one by one 
    python3 ./throughput.py
        ...
        --bands 5G_low,5G_high
    ==============================================================================="
         \nThroughput Under Channel Load Test Script for Tribands \n
         sudo python3 throughput_ver_2.py --mgr localhost --mgr_port 8080 --upstream_port eth1 
         --num_stations 40 --security open --ssid testchannel --passwd [BLANK] --test_duration 2 
         --ap_name WAC505 --ap_ip 192.168.208.13 --ap_password Password@123xzsawq@! --band triband 
         --vap_channel 11,36,100 --vap_radio wiphy0,wiphy1,wiphy2 --util 20-3000000,40-6000000 --threshold 80,50
         --upload 5000000 --download 5000000''')
        ...
        required.add_argument('--radio', help='Provide radio for different bands eg: wiphy3')
        optional.add_argument('--test_duration', help = 'Sets the duration of the test in minutes', default = 1)
        optional.add_argument('--bands', help = 'Select bands from [2.4G,5G]', default = '2.4G,5G_low,5G_high')
        required.add_argument('--vap_channel', help = 'To create VAP provide the AP channels of respective bands ', default = '100')
        required.add_argument('--vap_radio', help = 'VAP radio eg: wiphy2')
        optional.add_argument('--util', nargs="+", help = 'Channel utilization(provide whole number eg: 11,23,30,etc) with data_rate(bps) for that utilization',
                         default = "20-3000000,40-6000000")
        optional.add_argument('--threshold', help = 'Set the threshold for each utilization. '
                          'By default it will take 100-util_value (eg: 100-20=80) in case user not providing the Threshold')
        optional.add_argument('--ap_name', help =  'AP name')
        required.add_argument('--ap_ip', help =  'IP of AP which was connected')
        required.add_argument('--ap_password',help = 'Password for AP')
        optional.add_argument('--upload', help =  'Upload bps rate minimum for side_a of netgear', default = 0)
        optional.add_argument('--download', help =  'Download bps rate minimum for side_b of netgear', default = 0)
        args = parser.parse_args()

        bands = args.bands.title()
        ...
        for i in range(len(args.util)):
            util_list.append(int(float(args.util[i].split('-')[0])))
            rate_list.append(args.util[i].split('-')[1])

        num_sta = lambda ars: ars if (ars != None and ars != 0) else 40    # if num station is None by deafault it create 2 stations
        mode =  lambda ars: 11 if ars == '2.4G' else 9    # mode-9 (anAC) mode-11 (bgnAC)

        '''if ('Tribands' in bands or 'Triband' in bands):

            if len(bands) != 1:
                raise ValueError('Bands must be either Triband or [2.4G,5G_low,5G_high] but not both')

            elif len(bands) == 1:
                print(f"--- Bands to test : {bands} ---")
                test_time = datetime.datetime.now().strftime("%b %d %H:%M:%S")
                print("Test started at ", test_time)
                bands, station_list, vap_name = ['2.4G','5G_low','5G_high'], [], []

                for i in range(len(bands)):
                    station_list.append(LFUtils.portNameSeries(prefix_=f"{bands[i]}sta", start_id_=0, end_id_=3, padding_number_=100,
                                                          radio=args.radio[i]))
                    vap_name.append(f'vap{bands[i]}0')
                ...
                ip_var_test = IPV4VariableTime(host=args.mgr, port=args.mgr_port, number_template="0000",
                                               ...
                                               ssid="vap_ssid", password='[BLANK]', radio=args.radio[i],
                                               ...
                                               mode=mode(bands[i]), ap=args.ap_name,
                                               _vap_list=vap_name, _vap_radio=args.vap_radio[i], _dhcp=False)
                ...
                station_list1 = LFUtils.portNameSeries(prefix_="Thsta", start_id_=0,
                                                       end_id_=int(num_sta(args.num_stations)) - 1, padding_number_=10000,
                                                       radio=args.radio[i])
                ...
                ip_var_test1 = IPV4VariableTime(host=args.mgr, port=args.mgr_port, number_template="0000",
                                                ...
                                                ssid=args.ssid, password=args.passwd, radio=args.radio[i],
                                                security=args.security, test_duration=args.test_duration,
                                                use_ht160=False, side_a_min_rate=int(float(args.upload)),
                                                side_b_min_rate=int(float(args.download)),
                                                mode=args.mode, ap=args.ap_name, _dhcp=True)

                # check the channel utilization
                ip_var_test.check_util(real_cli_obj=ip_var_test1, util_list=util_list, real_cli=station_list1,
                                       ssh_root=args.ap_ip, ssh_passwd=args.ap_password, test_time=test_time,
                                       up_down=rate_list, threshold=threshold, channnel=int(float(args.vap_channel)))

                if not ip_var_test1.passes():
                    print(ip_var_test1.get_fail_message())
                    ip_var_test1.exit_fail()

        else:'''
        print(f"--- Bands to test : {bands} ---")
        #for i in range(len(bands)):
        if bands not in ['2.4G','5G']:
            raise ValueError(f"{bands} should be in ['2.4G','5G']")
        # 4 stations created under VAP by default
        station_list = LFUtils.portNameSeries(prefix_="sta", start_id_=0, end_id_=3, padding_number_=10000,
                                              radio=args.radio)
        # vap name
        vap_name = f'{bands.replace(".","point")}_vap0'
        print("List of stations under VAP--",station_list,'\nVAP name--',vap_name)
        # traffic data rate for stations under vap
        vap_sta_upload, vap_sta_download = int(float(rate_list[0])), int(float(rate_list[0]))
        rate_list.pop(0)
        ...
        ip_var_test = IPV4VariableTime(host=args.mgr,           port=args.mgr_port,         number_template="0000",
                                       ...
                                       use_ht160=False,         side_a_min_rate= vap_sta_upload,
                                       side_b_min_rate=vap_sta_download,
                                       mode=mode(bands),          ap=args.ap_name,
        ...
        station_list1 = LFUtils.portNameSeries(prefix_="Thsta", start_id_=0, end_id_=int(num_sta(args.num_stations))-1, padding_number_=10000,
                                               radio=args.radio)
        print(f"Station list for netgear AP when band is-- {bands}.....\n",station_list1)
        ip_var_test1 = IPV4VariableTime(host=args.mgr,          port=args.mgr_port,         number_template="0000",
                        sta_list=station_list1, name_prefix="Thrp",         upstream=args.upstream_port,
                        ...
                        mode=mode(bands),    ap=args.ap_name,   _dhcp = True)
