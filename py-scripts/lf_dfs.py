...
from Ap_commands import Netgear
...
def run():
    ...
    ap_command = Netgear(ip=args.ip, user=args.user, pswd=args.pswd)
        ...
        for channel_s in channel_values:
            ...
            if args.model == 'triband':
                if channel_s == "52" or channel_s == "56" or channel_s == "60" or channel_s == "64":
                    ap_command.fiveg_low_on_off(status="up")
                    ap_command.fiveg_high_on_off(status="down")
                    time.sleep(30)
                    ap_command.set_channel_in_ap_at_(channel=channel_s, model=args.model)
                    ...
                    ap_command.fiveg_low_on_off(status="down")
                    ap_command.fiveg_high_on_off(status="up")
                    time.sleep(30)
                    ap_command.set_channel_in_ap_at_(channel=channel_s, model=args.model)
                    print("channel set checking....")
                    logging.warning('channel set checking....')
            ...
            if monitoring_station == channel_s:
                ...
                ap_logs = ap_command.check_radar_time()
                N = 2
                ...
                if (str(res).__contains__("Radar detected on channel " + channel_s)):
                    ...
                    if data == "YES":
                        ...
                        if detecting_radar_channel == "-1" or detecting_radar_channel == "AUTO" or detecting_radar_channel == channel_s:
                            ...
                            logging.warning("Checking AP logs")
                            ap_logs_check = ap_command.check_radar_time()
